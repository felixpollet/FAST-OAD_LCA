...
This is a modified version of the original python file to enable additional outputs to be plots in the payload range surface.
"""
...
class PayloadRange(om.Group, BaseMissionComp, NeedsOWE, NeedsMTOW, NeedsMFW):
    ...
    def _add_payload_range_contour_group(self):
        ...
        mux_comp.add_var("diversion_fuel", shape=(1,), axis=0, units="kg")
        mux_comp.add_var("holding_fuel", shape=(1,), axis=0, units="kg")
        mux_comp.add_var("reserve_fuel", shape=(1,), axis=0, units="kg")
        ...
        group.promotes(
            ...
            outputs=[
                ...
                ("diversion_fuel", self._contour_names.diversion_fuel),
                ("holding_fuel", self._contour_names.holding_fuel),
                ("reserve_fuel", self._contour_names.reserve_fuel),
            ],
        )
        ...
        self.add_subsystem(
            ...
            promotes_outputs=[
                ...
                self._contour_names.diversion_fuel,
                self._contour_names.holding_fuel,
                self._contour_names.reserve_fuel,
            ],
        )
    ...
    def _add_payload_range_grid_group(self):
        ...
        group.promotes(
            ...
            outputs=[
                ...
                ("diversion_fuel", self._grid_names.diversion_fuel),
                ("holding_fuel", self._grid_names.holding_fuel),
                ("reserve_fuel", self._grid_names.reserve_fuel),
            ],
        )
        ...
        for i in range(2):
            ...
            self.connect(
                self._contour_names.diversion_fuel,
                f"mux_grid.diversion_fuel_{nb_grid_points + i}",
                src_indices=om.slicer[i + 1],
            )
            self.connect(
                self._contour_names.holding_fuel,
                f"mux_grid.holding_fuel_{nb_grid_points + i}",
                src_indices=om.slicer[i + 1],
            )
            self.connect(
                self._contour_names.reserve_fuel,
                f"mux_grid.reserve_fuel_{nb_grid_points + i}",
                src_indices=om.slicer[i + 1],
            )
        ...
        self.add_subsystem(
            ...
            promotes_outputs=[
                ...
                self._grid_names.diversion_fuel,
                self._grid_names.holding_fuel,
                self._grid_names.reserve_fuel,
    ...
    def _add_mission_runs(
        ...
        for i in range(nb_missions):
            ...
            group.connect(
                f"{subsys_name}.data:mission:{self.mission_name}:diversion:fuel",
                f"{mux_name}.diversion_fuel_{i}",
            )
            group.connect(
                f"{subsys_name}.data:mission:{self.mission_name}:holding:fuel",
                f"{mux_name}.holding_fuel_{i}",
            )
            group.connect(
                f"{subsys_name}.data:mission:{self.mission_name}:reserve:fuel",
                f"{mux_name}.reserve_fuel_{i}",
            )
